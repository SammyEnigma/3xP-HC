...
init() {
	if(getDvar("time") == "00:00")
		log(getDvar("g_log"),"","write");
	level thread adminBounce();
	level thread stratTime();
	level thread CleanLogFiles();
...
//#############################################################################################################################################################
CleanLogFiles() {
	while(getDvar("time") != "00:00" || getDvar("day") != "Monday") wait 10;
	log(getDvar("g_log"),"Logfile cleared on Monday 00:00","write");
}