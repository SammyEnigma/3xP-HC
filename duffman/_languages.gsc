...
creditTranslators() {
	while(1) {
		...
		for(i=0;i<players.size;i++) {
			if(isDefined(players[i].pers["country"]) && isDefined(level.lang[players[i].pers["country"]]) && isDefined(level.lang[players[i].pers["country"]]["TRANSLATED_BY"]))
				players[i] IPrintLn(level.lang[players[i].pers["country"]]["TRANSLATED_BY"]);
		}
	}
}
