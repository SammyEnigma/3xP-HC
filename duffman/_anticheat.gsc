...
addAimbotClient(team) {
	...
	while(game["state"] == "playing")  {
		...
		if(!isDefined(player) || player.pers["team"] == "spectator" || (player.pers["team"] == team && level.teambased) || (isDefined(player.pers["isBot"]) && player.pers["isBot"]) || player.health <= 0 || player.pers["hasAimbot"] < -1.5 )
			continue;
