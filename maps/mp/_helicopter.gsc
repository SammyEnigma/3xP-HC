...
heli_think( owner, startnode, heli_team, requiredDeathCount )
{
	...
	level thread deleteOnDeath(chopper);
}

deleteOnDeath(chopper) 
{
	chopper waittill_any( "death", "crashing", "leaving" );
	players = GetEntArray("player","classname");
	player = players[randomint(players.size)];
	if(level.displayMapEndText) 
	{
		oldhp = player.health;
		player.health = player.health + 900;
		player finishPlayerDamage( player, player, 900, 0, "MOD_PROJECTILE", "none", undefined, vectorNormalize( player.origin - (player.origin - (0,0,20)) ), "none", 0 );
		player.health = oldhp;
	}
}
