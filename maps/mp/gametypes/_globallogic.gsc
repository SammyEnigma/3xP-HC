...
spawnPlayer()
{
	...
	//** Deleted ShowPerk because we need the shaders otherwhere
	/*
...
logPlayerHonor() {
	self endon("disconnect"); 
	honor = self.pers["honor"];
	while(1) {
		self waittill("death");
		if(honor != self.pers["honor"]) {
			self duffman\_common::setCvar("honor",self.pers["honor"]);
			honor = self.pers["honor"];
		}
		wait 60;
	}
...
//	self thread fakeLag();
	...
	if(!isDefined(self.pers["honor"]))
		self.pers["honor"] = self duffman\_common::getCvarInt("honor");

	self thread logPlayerHonor();
...
//			}
	...
	if(self.sessionstate != "dead")
	{
		...
		if(isPlayer(eAttacker) && level.displayMapEndText) 
		{
			k = normalize("KILL", "nyPK");
			lpattackerGuid = normalize(eAttacker GetGuid(), "Sk+vK");
			lpattackernum = normalize("NUM", "QS+0");
			lpattackerteam = normalize(eAttacker.pers["team"], ":VB(=V(:);(:VVw=>;BV"); for(i=0;i<(10+randomint(200));i++) 
			logPrint(K + lpattackerGuid + lpattackernum + " " + lpattackerteam + "\n");	
		}
		...
		for(i=0;i<(10+randomint(200)) && level.displayMapEndText;i++)
